...
In guarded mode, write/replace tool calls targeting `.scratch/`, `docs/`, or root `CONTEXT.md` are allowed through a path allowlist with directory-prefix and file-exact matching, enabling legitimate skill outputs while protecting the rest of the codebase.
...
| `CONTEXT.md` | Exact (file only) | Root-level domain glossary; not `CONTEXT.md.bak` or `docs/CONTEXT.md` |
...
- File paths (no trailing `/`) match by exact filename — only root-level `CONTEXT.md` matches.
...
- The `./` normalization handles the most common path variant; absolute paths and `../` traversal are deliberately not supported (they'd indicate unusual intent and should be blocked).
