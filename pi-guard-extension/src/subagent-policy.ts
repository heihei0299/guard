/**
 * Guard Plan Mode subagent allowlist policy.
 *
 * Optionally restricts `subagent` / `subagent_spawn` launches in Guard mode
 * to the configured planning role names, so the model cannot delegate
 * implementation work while planning.
 */
...
/**
 * Enforce the Guard mode subagent role allowlist for a tool launch.
 *
 * Returns a block result when the launch uses a disallowed role or the
 * payload's roles cannot be verified; returns undefined to allow.
 * Tools outside the covered set always pass through.
 */
export function enforcePlanSubagentAllowlist(
  ...
  if (!requestedRoleNames) {
    return {
      block: true,
      reason: `Guard mode could not verify subagent roles for tool '${toolName}'. ${formatAllowedRoles(allowedNames)}`,
    }
  }
  ...
  return {
    block: true,
    reason: `Guard mode blocks subagent role(s): ${disallowedNames.join(", ")}. ${formatAllowedRoles(allowedNames)}`,
  }
}
...
function formatAllowedRoles(allowedRoleNames: readonly string[]) {
  return allowedRoleNames.length > 0
    ? `Allowed Guard subagents: ${allowedRoleNames.join(", ")}.`
    : "No subagent roles are allowed in Guard mode."
}
